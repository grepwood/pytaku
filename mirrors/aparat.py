...
class aparat_handler(object):
	def __is_mirror_dead(self, player_url):
		...
		self.__session.post(player_url, headers=self.__close_header)
		return text == self.__death_indicator
	...
	def __generate_secondary_url(self, primary_url):
		...
		self.__session.post(primary_url, headers=self.__close_header)
	...
	def __generate_direct_link(self, secondary_url):
		...
		soup = BeautifulSoup(self.__response.text, "html.parser")
		self.__session.post(secondary_url, headers=self.__close_header)
